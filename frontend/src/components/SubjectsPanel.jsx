...
export default function SubjectsPanel() {
  ...
  async function load() {
    try {
      const [s, struct] = await Promise.all([api.getSubjects(), api.getSchoolStructure()]);
      setSubjects(s);
      setStructure(struct);
    } catch (err) {
  ...
  async function handleSubjectImport(e) {
    const file = e.target.files[0];
    if (!file) return;
    ...
    try {
      const formData = new FormData();
      formData.append('file', file);
      const result = await api.importSubjects(formData);
      setSubjectImportResult(result);
      load();
    } catch (err) {
      setError(err.message);
    } finally {
  ...
  return (
    <div style={{ maxWidth: '560px' }}>
      ...
      <h3 style={{ fontSize: '0.938rem', fontWeight: 500, marginBottom: '8px', color: 'var(--text-primary)' }}>
        {t('bulkSubjectAssignTitle')}
      </h3>
      <p style={{ fontSize: '0.812rem', color: 'var(--text-secondary)', marginBottom: '10px' }}>
        {t('bulkSubjectAssignColumns')}
        <br />{t('bulkSubjectAssignExplain')}
      </p>
      <input type="file" accept=".xlsx" onChange={handleSubjectImport} disabled={subjectImporting} />
      {subjectImporting && <p style={{ fontSize: '0.812rem', color: 'var(--text-secondary)' }}>{t('importing')}</p>}
      {subjectImportResult && (
        <p style={{ fontSize: '0.812rem', color: '#2e7d32' }}>
          {t('created')} {subjectImportResult.created}, {t('skipped')} {subjectImportResult.skipped} ({t('alreadyExisted')}), {t('enrolled')} {subjectImportResult.enrolled}.
          {subjectImportResult.errors?.length > 0 && (
            <span style={{ color: '#c0392b', display: 'block' }}>
              {subjectImportResult.errors.length} {t('rowsHadErrors')}
              {subjectImportResult.errors.map((e, idx) => <span key={idx} style={{ display: 'block' }}>{e}</span>)}
            </span>
          )}
        </p>
      )}
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
        <select
          ...
          style={styles.input}
        >
          <option value="">{t('gradeOptionalPlaceholder')}</option>
          {structure.grades.map((g) => (
            <option key={g} value={g}>{g}</option>
          ))}
        </select>
        <select
          ...
          style={styles.input}
        >
          <option value="">{t('classroomOptionalPlaceholder')}</option>
          {structure.classrooms.map((c) => (
            <option key={c} value={c}>{c}</option>
          ))}
        </select>
